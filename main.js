...
document.addEventListener("DOMContentLoaded", () => {
  ...
  async function extractTextFromPDF(file) {
    ...
    for (let i = 1; i <= pdf.numPages; i++) {
      ...
      textContent +=
        textContentObj.items.map((item) => item.str).join(" ") + " ";
    }
    return textContent;
  }
...
function renderPreview(examData) {
  ...
  document.querySelector(".preview-header .badge").textContent =
    "Ready to Publish";
  document.querySelector(".preview-header .badge").style.background =
    "var(--success)";
...
async function showMockResults() {
  ...
  const GOOGLE_SHEET_URL =
    "https://script.google.com/macros/s/AKfycbwbm7hTa15Z6PT8HSMOKoUjiarsbvQwDcatGZuN8dZq2QiIUotilC40_1O8GLlGAj1Xfw/exec";
  let results = [];
  ...
  if (results.length === 0) {
    results = [
      {
        name: "System Message",
        exam: "Please set up your Google Sheet URL!",
        score: "0%",
        status: "Fail",
        date: "Present",
      },
